...
## Investment experience implementation — 2026-09-06

Status: DONE
Owner: Planner/Implementer
Chosen path: Direct implementation. Primary mode: Implementation. The completed predecessor used Research then experiment and is retained below as evidence.

### Requirement and scope
Implement the approved professional, progressively disclosed investment workspace, searchable instrument and purchase entry, and trustworthy return/profit analytics for Indian-market and existing portfolio assets. Zerodha is the preferred broker mapping. Automated trading, Trading Lab, order placement and live execution are excluded from this implementation. Preserve the existing Phase 8 quant-readiness seams without extending or removing them.

### Research and experiment steps
- [x] 1. Discover current investment UI and domain constraints; research primary product, calculation, API and regulatory sources. Sol research lanes completed; source inspection only, browser unavailable.
- [x] 2. Reconcile evidence and gaps; record canonical research in `investment-experience/report-source.md` and a compact claim ledger. Critical source/code claims independently spot-checked by Coordinator.
- [x] 3. Pass evidence to separate Experiment Agents for a reversible mockup and reproducible synthetic-data checks, with hypotheses and success thresholds defined first. GPT-5.5 UI prototype PASS; Sol catalogue hypothesis FAIL 12/13 by its frozen exact-name threshold; replay semantics and safety assertions PASS; live strategy promotion DISCARD.
- [x] 4. Independent QA reviews artifacts, interactions, calculations and consequential claims. PASS_WITH_RISKS for research/experiment scope.
- [x] 5. Present mockup and proposed sequence for user approval. User approved implementation on 2026-09-06, selected Zerodha as the preferred broker, and explicitly deferred the automated-trading system.

### Acceptance criteria
- [x] Overview hides specialist detail behind understandable navigation; asset, holding, analytics and trading branches are demonstrable in the isolated prototype.
- [x] Instrument search demonstrates auto-filled identity and an explicit ambiguity/missing-data fallback; actual trade price, quantity, dates and charges stay auditable. Experiment correctly exposes provider-name mismatch; unique non-exact selection policy remains an implementation question.
- [x] XIRR/TWR, realized/unrealized and net P&L have clear proposed scope, valuation dates and unavailable states; current implementation gaps are documented rather than represented as delivered.
- [x] Intraday proposal documents data, costs, risk controls, testing stages and current Indian regulatory/API constraints; no profitability or HFT capability claim is made.
- [x] Prototype is isolated, uses labeled synthetic data and is independently verified; no production files changed.

### Implementation acceptance criteria
- [ ] `/investments` is a quiet overview with no more than three primary metric cards, one primary chart, valuation date/data-quality status, and clear routes to Holdings, Performance, Activity and Add investment. Trade, lease and instrument-administration forms are absent from the overview.
- [ ] Holdings are searchable and filterable in a dedicated route; each result opens its existing durable instrument URL. Empty, loading/unavailable, stale-price and unpriced states remain explicit and no incomplete total is presented as complete.
- [ ] A holding page initially shows its summary and reveals Performance, Lots & tax, Activity, Income & leases and Settings through accessible navigation. Lease ownership is valued once; existing lease actions remain available only in the relevant holding branch.
- [ ] Performance separates invested amount, market value, unrealised P&L, portfolio-wide realised P&L, income, total investment gain, absolute return, XIRR and TWR. Every displayed metric carries an as-of/scope label; any metric that cannot be computed from authoritative ledger and valuation inputs returns a typed unavailable reason instead of zero or an invented number.
- [ ] Portfolio-wide realised P&L includes disposals from fully closed instruments. Income and TWR are exposed only after their repository inputs and boundary valuations are verified by tests; otherwise the production UI shows their precise unavailable reason.
- [ ] Add investment starts with local search by symbol/name/ISIN and shows name, exchange, type and provider mapping. Only a unique exact symbol or ISIN may be preselected; every non-exact name/prefix and every ambiguous result requires explicit confirmation. Manual historical entry remains available.
- [ ] Selecting a catalogue result autofills identity and listing fields, then asks for actual side, trade date, units, execution price, account/platform and itemised charges or tax withheld. It previews signed cash impact to paise and persists the selected canonical identity plus dated provider mappings without treating a provider token as permanent identity.
- [ ] The local catalogue refreshes at most daily from a free public master and supports an optional Zerodha instrument-dump mapping when configured. Zerodha's missing ISIN is not guessed: ISIN comes from an existing holding/import or an independently approved reference. Missing credentials or provider outages leave cached search/manual entry working and create no paid-API requirement.
- [ ] Existing Phase 8 venue/order/risk behavior is untouched. No Trading Lab route, automated trading worker, broker order call, credential UI or live execution capability is added in this slice.
- [x] Narrow tests, `npm run typecheck`, `npm run lint`, relevant `npm test -- <filter>` commands and `npm run build` pass; independent Sol QA records PASS_WITH_RISKS and the implementation status is DONE.

### Implementation context map
| Need | Authoritative file/section | Why it is needed |
|---|---|---|
| Approved navigation and metric semantics | `investment-experience/report-source.md`, `investment-experience/experiment.md` | Defines the progressive-disclosure hierarchy and corrected prototype behavior. |
| Experiment and QA limits | `investment-experience/data-strategy-experiment.md`, `investment-experience/qa.md` | Preserves explicit-review matching, unavailable metrics and the no-live-promotion boundary. |
| Financial calculations | `30-CALCULATIONS.md`, Phase 5 here, `src/domain/portfolio.ts` | Keeps exact money, XIRR/TWR and realised/unrealised semantics authoritative. |
| Provider identity | `40-MARKET-DATA.md`, `src/domain/pricing.ts` | Internal instrument identity remains separate from provider-specific codes. |
| Current application contracts | `src/app/investing.usecases.ts`, `src/infra/container.ts` | Existing valuation/returns outputs and dependency composition are the integration point. |
| Current UI | `app/(root)/investments/**`, `src/ui/instrument-hints.ts` | Existing actions and financial controls must remain usable while routes are reorganised. |
| Next 16 conventions | `node_modules/next/dist/docs/01-app/01-getting-started/05-server-and-client-components.md`, `01-app/02-guides/forms.md`, `01-app/03-api-reference/04-functions/use-search-params.md`, `01-app/02-guides/instant-navigation.md` | Pages stay server-first, interactive islands stay narrow, search params are awaited and mutations use server actions. |

### Implementation decisions and constraints
- The user approved implementation after reviewing the researched mockup. This amendment intentionally supersedes the pending-production wording above while retaining the completed research record.
- The first delivery is the investment workspace only. Trading Lab and all automated/live trading are deferred; Phase 8 remains an untouched future seam.
- Zerodha is the preferred broker label and mapping, not the canonical identity source. A catalogue row receives its own internal ID; ISIN is used when verified, while Zerodha exchange token/tradingsymbol stays a dated provider mapping because tokens can change or be reused.
- No paid API is required. Search reads the persisted local catalogue rather than calling a provider on each keystroke. A free public daily master can supply discovery/ISIN; an optional configured Zerodha dump enriches broker mappings. Cached data and manual entry are first-class fallback states.
- Provider-truncated names such as TCS never auto-select. Exact unique ISIN/symbol selection is allowed; all non-exact text matches require a visible user choice.
- This slice may show income, total gain or TWR as typed unavailable if exact source inputs are absent. It must repair the closed-holding realised aggregate and must not ship a cosmetic approximation. Broader accounting/storage work discovered by the financial agent is recorded as a later, separately bounded correction unless it is necessary to satisfy the observable UI contract.
- Existing user work is present in this plan and the untracked research packet. Every agent preserves it and edits only its assigned paths.
- Model routing follows the user's resource preference: Astra remains Coordinator only; Sol handles cross-cutting financial/catalogue reasoning and final QA; GPT-5.5 handles bounded UI and mechanical test tasks.

### Step-by-step implementation relay
- [x] 1. Define and verify the investment workspace financial read model — Owner: Backend/Financial Expert (Sol) — Files: `src/app/investing.usecases.ts`, `src/infra/container.ts`, `tests/investing-integration.spec.ts` — Contract: add one serialisable workspace/metric output with `AVAILABLE | UNAVAILABLE`, scope/as-of/provenance fields; aggregate realised gain over open and closed instruments; expose income and TWR only from verified inputs; do not alter domain formulas — Verify: `npm test -- investing-integration` PASS (1/1 file, 85 assertions, including native-foreign-price/no-FX aggregate regression); `npm run typecheck` PASS; `npm run lint -- --quiet` PASS.
- [x] 2. Add the persistent catalogue and provider-mapping boundary — Owner: Backend/Data Expert (Sol), after step 1 because `src/infra/container.ts` is shared — Files: `src/domain/instrument-catalog.ts`, `src/app/instrument-catalog.usecases.ts`, `src/infra/instrument-catalog.ts`, `src/infra/db/schema.ts`, `src/infra/repositories.ts`, `src/infra/container.ts`, `src/infra/db/migrations/0014_instrument_catalog.sql`, `src/infra/db/migrations/meta/_journal.json`, `tests/instrument-catalog.spec.ts`, `tests/instrument-catalog-integration.spec.ts` — Contract: internal catalogue ID, nullable verified ISIN, exchange listing, effective-dated provider mapping, source/fetched-at/checksum; daily free-public-master refresh; optional Zerodha CSV enrichment; cached local search; exact/confirm/manual match states; no secret in a response/log and no paid dependency — Verify: `npm test -- instrument-catalog` PASS (2/2 files, 40 assertions); `npm test -- schema` PASS (2/2 files, 65 assertions); `npm run typecheck` PASS; `npm run lint -- --quiet` PASS; `npm run db:migrate` applied pending 0013 and new 0014 locally; `npm run db:check` PASS (15/15 migrations, 48 tables, reference data present). A03 repair on 2026-09-06 added `deleted_at` to all five catalogue tables and filtered tombstones from catalogue reads.
- [x] 3. Build search-first add-and-record flow — Owner: Frontend Expert (GPT-5.5), after step 2 — Files: `app/(root)/investments/new/page.tsx`, `app/(root)/investments/new/instrument-search.tsx`, `app/(root)/investments/new/record-investment-form.tsx`, `app/(root)/investments/catalog-actions.ts`, `tests/investment-entry.spec.ts` — Contract: keyboard-operable combobox with explicit confirmation; selected identity summary; manual fallback; actual date/side/quantity/unit price/account/platform/charges/TDS fields; signed paise cash preview; server validation and existing AddInstrument/RecordBuy/RecordSell use cases; no quote used as an execution price; tests cover exact-versus-confirm matching, BUY/SELL signs, paise and invalid input — Verify: `npm test -- investment-entry` PASS (1/1 file, 24 assertions); `npm run lint -- --quiet` PASS; `npm run typecheck` PASS.
      2026-09-06 Step 3 implementation evidence: route, combobox/listbox, record form, catalogue actions and focused spec are written. QA repair added the production due-refresh trigger before catalogue search; the refresh use case preserves 24-hour suppression and cached/manual fallback. Required commands are green.
- [x] 4. Replace the crowded root with the modular workspace — Owner: Frontend Expert (GPT-5.5), after step 1; may run in parallel with step 3 because paths are disjoint — Files: `app/(root)/investments/layout.tsx`, `app/(root)/investments/investment-nav.tsx`, `app/(root)/investments/page.tsx`, `app/(root)/investments/holdings/page.tsx`, `app/(root)/investments/holdings/holdings-table.tsx`, `app/(root)/investments/performance/page.tsx`, `app/(root)/investments/performance/performance-chart.tsx`, `app/(root)/investments/activity/page.tsx`, `app/(root)/investments/allocation-dashboard.tsx`, `tests/investment-workspace-ui.spec.ts` — Contract: server-first route data and narrow client islands; maximum three overview metrics/one chart; URL-addressable branches; compact searchable/filterable holdings; metric-specific performance panels and tables; stale/unpriced/unavailable states; activity links to durable records; tests assert exposure limits and route reachability — Verify: `npm test -- investment-workspace-ui` PASS (1/1 file, 48 assertions, including FX conversion state/filter and route parser distinct from native unpriced); scoped `npx eslint --quiet app/(root)/investments/holdings/page.tsx tests/investment-workspace-ui.spec.ts` PASS; `npm run typecheck` PASS.
- [x] 5. Apply progressive disclosure to holding detail and relocate lease controls — Owner: Frontend Expert (GPT-5.5), after step 4 — Files: `app/(root)/investments/[instrumentId]/page.tsx`, `app/(root)/investments/[instrumentId]/holding-nav.tsx`, `tests/investment-holding-ui.spec.ts` — Contract: awaited `searchParams` selects Summary, Performance, Lots & tax, Activity, Income & leases or Settings; default renders Summary only; existing trade, tax, gold analytics, lease and administration components render under their matching branch; invalid view falls back safely; valuation remains single-counted; tests cover branch/fallback behavior — Verify: `npm test -- investment-holding-ui`, `npm run lint -- --quiet`, `npm run typecheck`.
      2026-09-06 Step 5 implementation evidence: holding detail now reads awaited `view` search params through `holding-nav.tsx`, defaults invalid/missing values to Summary, and exposes URL-addressable Summary, Performance, Lots & tax, Activity, Income & leases and Settings branches. Existing components were relocated only: gold returns/charts/benchmark to Performance; lots, method comparison, lot ladder and tax statement to Lots & tax; trade history/forms and corporate actions to Activity; lease deployment/table/actions/open form to Income & leases for digital gold; instrument administration to Settings. Lease ownership and value remain sourced from the existing open-position and lease read models, with no added lease-value recomputation. Repair after QA defect: non-digital Performance now renders a nonblank panel with existing holding cost, market value, unrealised and realised figures plus the route-contract unavailable state for holding-level XIRR/TWR; non-gold Income & leases renders a nonblank not-applicable panel without lease controls. `npm test -- investment-holding-ui` PASS (1/1 file, 42 assertions after repair); `npm run lint -- --quiet` PASS; `npm run typecheck` PASS; scoped Step 5 `git diff --check` PASS.
- [x] 6. Independent acceptance review — Owner: Testing/QA Agent (Sol, never an implementing agent), after Coordinator integration — Files: read all changed files; write only this plan's QA record and a defect baton if needed — Contract: Coordinator first resolves interfaces and runs `npm run lint`, `npm run typecheck`, `npm test`, `npm run build`; QA then independently inspects and reruns acceptance/regression commands, exercises keyboard/responsive/visual behavior if a supported browser is available, and records `PASS`, `PASS_WITH_RISKS` or `FAIL` — Verify: QA evidence below; only PASS/PASS_WITH_RISKS permits `DONE`.

Dependencies: 1 -> 4 -> 5; 2 -> 3; 3–5 -> Coordinator integration -> 6. Steps 3 and 4 may run concurrently after their prerequisites because they own disjoint paths. Any correction to a shared file returns to the original owner and then back to QA.

### Implementation handoff
## Baton: Planner/Implementer -> Backend/Financial Expert
- Goal: Deliver the truthful serialisable read model that every investment route can consume.
- Completed: User approval, execution mode, Zerodha preference, automation exclusion, acceptance criteria, file ownership, dependencies and Next 16 constraints are recorded above.
- Decisions: Closed holdings participate in realised P&L; missing income/TWR inputs produce typed unavailable results; provider and UI work wait on stable read/data contracts.
- Inputs: This section; `investment-experience/report-source.md`; `investment-experience/qa.md`; `src/app/investing.usecases.ts`; `src/domain/portfolio.ts`; `tests/investing-integration.spec.ts`.
- Changed files: `_architecture/70-UPGRADE-PLAN.md` only for this planning baton.
- Contract/output: Step 1's workspace financial read model and focused integration evidence.
- Verification: Planner inspected the existing valuation/returns contracts and local Next 16 server/client, forms, search-params and navigation guides; no production command is claimed yet.
- Open risks: Exact income source and historical boundary valuations may be absent; report that as typed unavailability rather than widening scope or approximating.
- Next action: Execute step 1, update its checkbox/evidence, and pass the output contract to step 4.
- Do not revisit: Automated/live trading is excluded; Zerodha tokens are mappings; no paid API; no fuzzy auto-selection; no invented historic fills, charges, income or TWR.

## Baton: Backend/Financial Expert -> Frontend Expert
- Goal: Consume the truthful route-facing investment workspace read model.
- Completed: Added `InvestmentWorkspace`, fixed lifetime realised P&L to include disposals from fully closed instruments, wired `services().investing.workspace`, covered JSON serialization plus unavailable income/TWR, and repaired position-level native-price versus reporting-FX availability.
- Decisions: Exact money and percentages cross the route boundary as decimal strings. Income is `UNAVAILABLE/INCOME_INPUTS_UNVERIFIED`; total gain remains unavailable with it; TWR is `UNAVAILABLE/TWR_BOUNDARY_VALUATIONS_UNAVAILABLE` until repository boundary valuations exist.
- Inputs: `src/app/investing.usecases.ts` exports `InvestmentWorkspace`, `InvestmentWorkspaceOutput`, `InvestmentWorkspacePosition`, `InvestmentMetric`, `InvestmentMetricContext`, and `InvestmentMetricUnavailableReason`; call `services().investing.workspace.execute({ userId, asOf })`.
- Changed files: `src/app/investing.usecases.ts`; `src/infra/container.ts`; `tests/investing-integration.spec.ts`; this Step 1 record in `_architecture/70-UPGRADE-PLAN.md`.
- Contract/output: `InvestmentWorkspaceOutput` has primitive-only `asOf`, `reportingCurrency`, `positions`, `dataQuality`, and metrics `investedAmount`, `marketValue`, `unrealisedPnl`, `realisedPnl`, `income`, `totalInvestmentGain`, `absoluteReturn`, `xirr`, `twr`. Every metric is discriminated by `status: AVAILABLE | UNAVAILABLE` and carries `unit`, `currency`, `scope: PORTFOLIO`, `period: { from: INCEPTION, through }`, `asOf`, and `provenance`; available metrics add `value`, unavailable metrics add `reason` and `message`. Each emitted position now includes optional-for-source-compatibility `marketValueStatus`, with runtime values `AVAILABLE`, `NATIVE_PRICE_UNAVAILABLE`, or `REPORTING_CURRENCY_CONVERSION_UNAVAILABLE`; `unavailableReason` names the matching cause, while `isStale` remains reserved for an actually stale native quote or resolved FX observation.
- Verification: `npm test -- investing-integration` PASS (1/1 file, 85 assertions); `npm run typecheck` PASS; `npm run lint -- --quiet` PASS. Missing FX is present only in `unconvertedPositions`, absent from `unpricedPositions` and `stalePositions`; native-price and resolved-FX staleness remain independently preserved.
- Open risks: Investment income attribution and flow-boundary valuation history do not exist in the verified repositories, so income, total gain and TWR intentionally remain unavailable.
- Next action: Render the workspace metrics and positions with explicit unavailable, partial, stale and unpriced states; never substitute zero for an unavailable metric.
- Do not revisit: Domain formulas, automated trading, and the catalogue/provider contract were untouched.

## Baton: Backend/Data Expert -> Frontend Expert
- Goal: Consume the persistent local catalogue in the search-first add-investment flow without calling providers per keystroke.
- Completed: Added canonical catalogue identities, exchange listings, append-preserving effective-dated provider mappings, successful-fetch provenance, durable links from user-owned instruments, a daily Upstox public-master refresh, optional configured Zerodha CSV enrichment, cached local search, and resilient manual/cache fallback. The TCS long-name query returns the provider-truncated row for explicit confirmation; it never preselects. The later A03 repair added nullable tombstones to all five catalogue tables and excludes deleted catalogue rows from reads; the integration suite proves a deleted listing disappears from search.
- Decisions: A unique exact normalized ISIN or symbol is the only `EXACT` state. Exact names, truncated-name candidates, prefixes and ambiguity return `CONFIRM`; no candidates return `MANUAL`. Zerodha and Upstox tokens remain mapping fields. The raw bulk response is hashed and discarded; only normalized private-cache rows and fetch receipts persist.
- Inputs: `src/domain/instrument-catalog.ts`; `src/app/instrument-catalog.usecases.ts`; `services().instrumentCatalog` in `src/infra/container.ts`; Step 2 tests.
- Changed files: `src/domain/instrument-catalog.ts`; `src/app/instrument-catalog.usecases.ts`; `src/infra/instrument-catalog.ts`; `src/infra/db/schema.ts`; `src/infra/repositories.ts`; `src/infra/container.ts`; `src/infra/db/migrations/0014_instrument_catalog.sql`; `src/infra/db/migrations/meta/_journal.json`; `tests/instrument-catalog.spec.ts`; `tests/instrument-catalog-integration.spec.ts`; this Step 2 record.
- Contract/output: Call `services().instrumentCatalog.search.execute({ query, limit? })`. It returns primitive-only `{ query, matchState: "EXACT" | "CONFIRM" | "MANUAL", selected, candidates, manualEntryAllowed: true, cache }`. `selected` is non-null only for one exact symbol/ISIN. Each candidate has `{ catalogInstrumentId, isin, name, instrumentType, listing: { id, exchange, segment, symbol, name, currency }, providerMappings: [{ provider, providerInstrumentId, providerToken, tradingSymbol, effectiveFrom }] }`. `cache` has `{ status: "READY" | "EMPTY", source, fetchedAt, checksum, stale }`. Call `services().instrumentCatalog.refresh.execute()` before search when desired; it returns `REFRESHED | CURRENT | CACHE_FALLBACK | EMPTY_MANUAL_ONLY` plus source outcomes and never makes more than one provider attempt per rolling 24 hours. After `addInstrument` succeeds, call `services().instrumentCatalog.link.execute({ userId, portfolioInstrumentId: result.instrumentId.value, catalogInstrumentId, listingId })`; `{ linked: true }` confirms the canonical identity was persisted.
- Verification: `npm test -- instrument-catalog` PASS (2/2 files, 40 assertions); `npm test -- schema` PASS (2/2 files, 65 assertions); `npm run typecheck` PASS; `npm run lint -- --quiet` PASS; `npm run db:check` PASS (15/15 migrations, all 48 declared tables, reference data present). Earlier scoped `git diff --check` PASS preceded the A03 evidence-only amendment.
- Open risks: The shipped public parser intentionally covers NSE cash `EQ` rows only. The optional Zerodha enrichment requires server-side `ZERODHA_API_KEY` and `ZERODHA_ACCESS_TOKEN`; missing values simply omit that provider. Catalogue use remains private/cache-only because redistribution rights are unresolved, and live upstream availability was not used as test evidence.
- Next action: Build Step 3 against the exact contract above, keeping actual execution price and itemised charges user/import supplied rather than sourced from catalogue metadata.
- Do not revisit: `services().investing.workspace`, legacy investment wiring, Phase 8 venue/order/risk behavior, no-paid-provider policy, exact-only preselection, or the no-live-order boundary.

## Baton: Frontend Expert -> Coordinator / Testing QA Agent
- Goal: Verify and integrate the search-first add-and-record investment flow.
- Completed: Added `/investments/new`, a keyboard-operable catalogue combobox/listbox with explicit identity confirmation, manual historical entry fallback, selected identity summary, actual trade date/side/quantity/execution price/account/platform/itemised charge/TDS fields, signed INR paise cash preview, and server actions that trigger due refresh before production search, re-check catalogue selections before recording, then call existing `addInstrument`, `recordBuy`/`recordSell`, and catalogue `link` use cases.
- Decisions: Exact catalogue symbol/ISIN may stage automatically but still requires the visible "Use this identity" confirmation before submit; every CONFIRM candidate and manual entry requires user choice. Catalogue data supplies identity/listing only, never execution price. TDS is collected as an itemised cash-impact component and persisted through the existing aggregate trade-charge contract because the current buy/sell use cases have no separate trade-TDS ledger leg inside Step 3 scope.
- Inputs: Step 3 files; Backend/Data catalogue baton above; local Next forms, server/client component and `use server` directive docs.
- Changed files: `app/(root)/investments/catalog-actions.ts`; `app/(root)/investments/new/page.tsx`; `app/(root)/investments/new/instrument-search.tsx`; `app/(root)/investments/new/record-investment-form.tsx`; `tests/investment-entry.spec.ts`; this Step 3 evidence and baton in `_architecture/70-UPGRADE-PLAN.md`.
- Contract/output: Search calls `services().instrumentCatalog.refresh.execute()` before `services().instrumentCatalog.search.execute({ query, limit: 12 })`; refresh failures are not returned by the search action, leaving the searched cache/manual response usable. Form posts primitive strings to `recordInvestmentEntryAction`; the server validates identity, dates, positive quantity/price and itemised money fields, re-resolves catalogue selections from `services().instrumentCatalog.search`, registers or reuses the instrument, persists the catalogue link when applicable, and records BUY/SELL through existing investing use cases. Successful state returns `{ ok, message, instrumentId, cashPreview }`.
- Verification: `npm test -- investment-entry` PASS (1/1 file, 24 assertions, including production search refresh order plus cached/manual fallback on provider failure). `npm run typecheck` PASS. `npm run lint -- --quiet` PASS.
- Open risks: Browser visual QA has not run here. Separate trade-TDS accounting is not represented by existing RecordBuy/RecordSell contracts.
- Next action: Coordinator sends Step 3 to independent QA with browser keyboard checks for the combobox and form.
- Do not revisit: Catalogue matching policy, provider/token identity separation, live order exclusion, and no paid per-keystroke provider calls.

## Baton: Frontend Expert -> Coordinator / Testing QA Agent
- Goal: Verify and integrate Step 4's modular investment workspace.
- Completed: Added `/investments` nested layout and active nav; replaced the crowded root with a quiet server-first overview fed by `services().investing.workspace`; moved holdings, performance and activity into URL-addressable routes; removed root trade, lease and admin forms; added `tests/investment-workspace-ui.spec.ts`; repaired the holdings table and route parser to distinguish native price absence from reporting-currency conversion absence via `marketValueStatus`.
- Decisions: Overview shows exactly three primary metrics (`marketValue`, `unrealisedPnl`, `realisedPnl`) and one allocation chart. Holdings filtering is query-param based with a server-rendered form. Performance renders income, total gain and TWR unavailable messages from the read model instead of charting or displaying zero. A null reporting value is labelled from `marketValueStatus`: `NATIVE_PRICE_UNAVAILABLE` is native unpriced, while `REPORTING_CURRENCY_CONVERSION_UNAVAILABLE` keeps the native currency/priced-on observation visible and states that FX conversion is missing. The holdings route-level parser accepts the matching `fx-unavailable` query value so the URL form cannot fall back to all rows.
- Inputs: Step 1 `InvestmentWorkspaceOutput` contract in `src/app/investing.usecases.ts`; Step 4 file scope; local Next server/client, navigation, instant-navigation and search-params docs.
- Changed files: `app/(root)/investments/layout.tsx`; `app/(root)/investments/investment-nav.tsx`; `app/(root)/investments/page.tsx`; `app/(root)/investments/holdings/page.tsx`; `app/(root)/investments/holdings/holdings-table.tsx`; `app/(root)/investments/performance/page.tsx`; `app/(root)/investments/performance/performance-chart.tsx`; `app/(root)/investments/activity/page.tsx`; `app/(root)/investments/allocation-dashboard.tsx`; `tests/investment-workspace-ui.spec.ts`; this Step 4 evidence and baton.
- Contract/output: Root overview exposes valuation date/data-quality, no incomplete complete total, no root trade/lease/admin forms, durable links to Holdings/Performance/Activity/Add investment, a searchable/filterable holdings route, metric-specific performance panels, and compact activity links to realised history and durable holding records.
- Verification: `npm test -- investment-workspace-ui` PASS (1/1 file, 48 assertions, including `marketValueStatus` FX conversion repair and route parser coverage); scoped `npx eslint --quiet app/(root)/investments/holdings/page.tsx tests/investment-workspace-ui.spec.ts` PASS; `npm run typecheck` PASS.
- Open risks: Browser visual QA has not run here; Step 5 still owns holding-detail progressive disclosure and lease control relocation.
- Next action: Coordinator sends integrated Steps 3 and 4 to independent QA after any final whole-tree acceptance commands.
- Do not revisit: Domain formulas, Step 3 add-entry internals, holding detail files, lease controls, Trading Lab, automated trading, broker order placement, or live execution.

## Baton: Frontend Expert -> Coordinator / Testing QA Agent
- Goal: Verify and integrate Step 5's holding-detail progressive disclosure.
- Completed: Added `holding-nav.tsx`; updated the holding detail page to read awaited `searchParams.view`; Summary is the default/fallback view; existing sections now render under Summary, Performance, Lots & tax, Activity, Income & leases and Settings. Repaired QA defect so non-digital Performance and non-gold Income & leases render nonblank content.
- Decisions: Used server-page `searchParams` rather than a client hook, with `Link` URLs for durable branch navigation. Lease deployment and lease actions are visible only in the Income & leases branch for the existing digital-gold lease path; no lease ownership/value math was added. Non-digital Performance uses the already-loaded holding cost, market value, unrealised and realised figures, and states holding-level XIRR/TWR are unavailable in this route contract.
- Inputs: Step 5 files; Step 4 baton above; local Next server/client, navigation and `use-search-params` docs.
- Changed files: `app/(root)/investments/[instrumentId]/page.tsx`; `app/(root)/investments/[instrumentId]/holding-nav.tsx`; `tests/investment-holding-ui.spec.ts`; this Step 5 evidence and baton in `_architecture/70-UPGRADE-PLAN.md`.
- Contract/output: `/investments/[instrumentId]` renders Summary for missing or invalid `view`; `/investments/[instrumentId]?view=performance|lots-tax|activity|income-leases|settings` renders the matching branch with accessible nav and `aria-current`. Non-digital Performance and non-gold Income & leases are nonblank; non-gold Income & leases contains no lease controls. Existing trade, tax, gold analytics, lease and admin components were not edited.
- Verification: `npm test -- investment-holding-ui` PASS (1/1 file, 42 assertions after repair); `npm run lint -- --quiet` PASS; `npm run typecheck` PASS; scoped Step 5 `git diff --check` PASS.
- Open risks: Browser visual/responsive/keyboard QA has not run here; this spec is a source/render contract check.
- Next action: Coordinator sends the implementation set to independent Testing/QA for Step 6 acceptance.
- Do not revisit: Domain formulas, child component internals, Step 3 add-entry internals, workspace routes from Step 4, automated trading, broker order placement, live execution, or lease valuation semantics.

### Implementation QA record
Status: PASS_WITH_RISKS
Evidence:
- Coordinator integration complete: `git diff --check` PASS; `npm run lint -- --quiet` PASS; `npm run typecheck` PASS; `npm run db:check` PASS (15/15 migrations, 48 tables); `npm test` PASS (55/55 spec files); `npm run build` PASS and generated all investment routes.
- Initial full-suite run found A03 soft-delete failures for the five catalogue tables. The Step 2 owner added nullable `deleted_at` columns to schema/migration, filtered catalogue reads, and added a tombstoned-listing integration assertion. The full suite passed after repair.
- Supported browser discovery returned an empty browser list, so Coordinator could not perform visual/responsive/keyboard interaction QA.
- Independent QA reran `npm test -- investment-entry` PASS (1/1 file, 15 assertions), `npm test -- investment-workspace-ui` PASS (1/1 file, 43 assertions), `npm test -- investment-holding-ui` PASS (1/1 file, 36 assertions), `npm test -- instrument-catalog` PASS (2/2 files, 40 assertions), `npm test -- investing-integration` PASS (1/1 file, 77 assertions), `npm run typecheck` PASS, `npm run lint -- --quiet` PASS, and `npm run db:check` PASS (15/15 migrations, 48 tables, reference data present).
- Independent source review found three acceptance defects: non-digital holding Performance and Income & leases branches render blank; valid native prices with unavailable FX conversion are mislabeled and filtered as unpriced; and no production caller invokes `instrumentCatalog.refresh`, so a fresh catalogue remains empty and search falls back to manual indefinitely.
- Independent `git diff HEAD --check` FAIL: four trailing-whitespace lines in `_architecture/investment-experience/data-strategy-experiment.md` contradict the recorded whole-tree PASS. This artifact belongs to the separate research/experiment packet but the command evidence must be corrected.
- Repair review confirmed non-digital Performance and Income & leases branches now render explicit content without exposing lease controls outside digital gold; `npm test -- investment-holding-ui` PASS (1/1 file, 42 assertions).
- Repair review confirmed native-price absence and reporting-FX conversion absence are separate position states and filters, `fx-unavailable` is accepted by the route parser, and missing FX is excluded from aggregate stale positions; `npm test -- investment-workspace-ui` PASS (1/1 file, 48 assertions) and `npm test -- investing-integration` PASS (1/1 file, 85 assertions).
- Repair review confirmed production catalogue search invokes the due-gated refresh first, while provider failure preserves cached search or manual-only fallback without returning provider error detail; `npm test -- investment-entry` PASS (1/1 file, 24 assertions).
- Final independent checks: `npm run typecheck` PASS; scoped ESLint over all repaired production/test files PASS; `git diff HEAD --check` PASS with line-ending warnings only. Coordinator reran the full `npm test` suite (55/55 files), `npm run lint -- --quiet`, `npm run typecheck`, `npm run db:check` (15/15 migrations, 48 tables), and `npm run build` (Next 16.3.3, all investment routes) successfully before the final narrow stale-list predicate repair, then reran its focused integration test, typecheck and diff check successfully afterward.
Residual risks:
- Browser visual/responsive/keyboard QA was NOT_RUN because supported browser discovery returned an empty list; structural accessibility and responsive contracts passed source-level tests.
- Catalogue licensing/redistribution remains limited to private application use and cached metadata; no bulk snapshot may be committed or published.
- Income, total gain and TWR intentionally remain typed unavailable until verified income attribution and boundary valuations are implemented.
- Separate trade-TDS accounting is not represented by the current RecordBuy/RecordSell contracts. The entry form captures tax withheld separately, includes it in signed cash impact, warns that tax withheld and STT are usually non-deductible, and persists it inside aggregate trade charges.

## Historical baton: Testing/QA Agent -> Coordinator / repair owners (initial FAIL)
- Goal: Return the implementation for the smallest repairs needed before a fresh independent Step 6 review.
- Completed: Reviewed the active implementation plan, all implementation diffs and contracts; independently reran the targeted financial, catalogue, entry, workspace and holding suites plus typecheck, lint and database checks.
- Decisions: FAIL. Step 6 remains unchecked and the plan remains in QA because three observable acceptance defects remain.
- Inputs: This implementation section; `src/app/investing.usecases.ts`; `src/app/instrument-catalog.usecases.ts`; `src/infra/instrument-catalog.ts`; `src/infra/repositories.ts`; `app/(root)/investments/**`; focused investment and catalogue specs.
- Changed files: `_architecture/70-UPGRADE-PLAN.md` only (QA record and this baton).
- Contract/output: Defect 1 — for a non-digital holding, open `?view=performance` or `?view=income-leases`; expected branch content or an explicit unavailable/not-applicable state, actual header/nav followed by blank content. Smallest owner: Step 5 Frontend Expert, holding page and focused test. Defect 2 — create a foreign holding with a valid native quote but unavailable reporting FX; expected a distinct FX/unconverted unavailable row and filter state, actual `Unpriced / No price resolved` and inclusion in the unpriced filter. Smallest owner: Step 1 read-model mapping plus Step 4 holdings UI/tests. Defect 3 — on a fresh migrated database, open Add investment and search `TCS`; expected a due refresh attempt with cached/manual fallback on failure, actual no production invocation of `instrumentCatalog.refresh`, leaving the catalogue empty indefinitely. Smallest owner: Step 3 server integration with the Step 2 refresh contract and a focused integration test.
- Verification: Targeted suites, typecheck, lint and `db:check` PASS as recorded above. Coordinator `npm test` and `npm run build` PASS were reviewed but not duplicated. `git diff HEAD --check` FAIL on four trailing-space lines in the separate experiment Markdown. Browser visual/responsive/keyboard checks NOT_RUN because supported browser discovery returned an empty list and the assignment prohibited substitute tooling.
- Open risks: Separate trade-TDS accounting is absent from the current RecordBuy/RecordSell contract; the form warns that STT and tax withheld are usually non-deductible, but persists tax withheld inside aggregate trade charges. Catalogue redistribution remains private/cache-only. Income, total gain and TWR remain intentionally unavailable pending verified repository inputs.
- Next action: Repair the three defects, correct or accurately scope the diff-check evidence, rerun focused regression checks, then return the integrated result to a fresh independent Step 6 QA run.
- Do not revisit: Closed-holding realised P&L, exact/confirm/manual matching policy, canonical/provider identity separation, signed BUY/SELL paise math, lease single-counting, and the no-live-trading boundary all passed source and targeted automated review.

## Baton: Testing/QA Agent -> Coordinator (final acceptance)
- Goal: Deliver the repaired investment experience after independent Step 6 acceptance review.
- Completed: Re-inspected the repaired financial read model, holding branches, holdings filters/parser, production catalogue refresh flow and focused tests; reran proportionate regression and static checks.
- Decisions: PASS_WITH_RISKS. All observable implementation acceptance criteria pass. Step 6 is checked and this implementation section is DONE.
- Inputs: This implementation section and historical FAIL baton; repaired files in `src/app/investing.usecases.ts`, `app/(root)/investments/catalog-actions.ts`, `app/(root)/investments/holdings/**`, `app/(root)/investments/[instrumentId]/page.tsx`, and their focused specs.
- Changed files: `_architecture/70-UPGRADE-PLAN.md` only for final QA status, Step 6, evidence and this baton.
- Contract/output: Non-digital holding branches are nonblank; native unpriced, stale and reporting-FX-unavailable states remain distinct through the read model, aggregate, route parser, filter and row copy; search performs the persisted 24-hour due-gated refresh before local lookup and preserves cache/manual fallback; all previously accepted financial, catalogue identity, paise calculation, lease single-count and no-live-trading boundaries remain intact.
- Verification: Independent `npm test -- investment-holding-ui` PASS (42 assertions), `npm test -- investment-entry` PASS (24 assertions), `npm test -- investment-workspace-ui` PASS (48 assertions), `npm test -- investing-integration` PASS (85 assertions), `npm run typecheck` PASS, scoped ESLint PASS, and `git diff HEAD --check` PASS. Coordinator whole-tree lint, typecheck, database check, 55/55 test files and Next production build PASS; final narrow repair was followed by focused test, typecheck and diff-check PASS. Browser visual/responsive/keyboard QA NOT_RUN because no supported browser was available.
- Open risks: Browser interaction remains unverified; trade TDS remains an itemised input folded into aggregate trade charges; catalogue redistribution remains private/cache-only; income, total gain and TWR remain explicitly unavailable pending authoritative inputs.
- Next action: Coordinator delivers the completed implementation and its residual risks to the user.
- Do not revisit: The three initial QA defects and the later aggregate stale/unconverted overlap are repaired and independently verified.

### Context map and routing
Plan of record: this section. Domain references: `30-CALCULATIONS.md`, `40-MARKET-DATA.md`, Phase 5 and Phase 8 here. Research artifacts live under `_architecture/investment-experience/`; these support this plan and do not replace it. Existing completed phase decisions remain authoritative; any proposed departure is a proposal pending approval.

Research uses official broker/exchange/regulator documentation, first-party product guidance and calculation references, with access dates, limitations and targeted follow-up. Planning tool is unavailable in the current tool catalog; this checklist records the required research plan instead.

### Handoffs and QA
Coordinator -> Research: bounded UX/analytics and broker/automation lanes, Sol agents; no production edits or recursive delegation. Coordinator owns evidence synthesis. User's model-routing clarification: Astra for heavy reasoning, Sol/GPT-5.5 for bounded tasks. Experiment: GPT-5.5 UI, Sol data/strategy in disjoint files after each research handoff. QA: independent non-implementing Sol agent.
QA status: PASS_WITH_RISKS. Evidence: `investment-experience/qa.md`; UI command exits 0; data command intentionally exits 1 on the frozen 12/13 catalogue threshold while replay/safety assertions pass. Residual risks: browser visual/responsive/keyboard checks were unavailable; provider name truncation and unique-prefix review policy remain; broker, market scope, latency target and user preferences are unconfirmed; synthetic prototype cannot prove usability or live trading performance.

### Experiment protocol (defined before running)
1. Progressive disclosure: default overview has at most three primary metrics, one chart, and no trade, lease or instrument administration forms. Holdings, holding detail, performance, activity and Trading Lab are reachable within two navigation actions from overview. Measure the prototype's navigation graph and rendered view state; this is a structural proxy, not a human usability study.
2. Search-first entry: a local fixture catalogue supports name/symbol search, exchange disambiguation and identifier autofill without manual ISIN entry. Unknown queries offer an explicit fallback. No live search API integration is implied by the fixture.
3. Financial clarity: deterministic fixtures verify a one-year 10% XIRR, cashflow-neutral TWR, gross-to-net trading costs, and no invented total when a required quote is missing. Charts must use labeled synthetic values with consistent dates and scope.
4. Automation safety: an isolated simulated gate blocks stale data, loss-limit breaches and repeated order intents; no network order calls. Measure results, not strategy profitability. Strategy parameters are examples for later user refinement.
5. QA independently runs available structural/runtime checks. Browser connection currently returns `No browser is available` and an empty browser list; visual/keyboard/responsive QA remains unverified unless a supported connection becomes available.

Proposed departure pending approval: lease management may become a holding-level branch while ownership and valuation remain counted exactly once. Phase 9's decision to keep lease UI directly on `/investments` remains the production behavior until approval. A future persistent trading execution worker would similarly require an explicit architecture decision; Phase 8 currently provides a simulated venue only.

### Research relay record
- Goal: separate investment navigation and trustworthy metrics from intraday strategy operations.
- Completed: two independent Sol lanes reviewed product/calculation and provider/regulatory primary evidence. Coordinator verified critical sources and current income/realized-gain code.
- Decisions: internal canonical instrument identity plus dated provider mappings; search-first trade entry; economic P&L separate from taxable gain; dates and missing-data states visible; Trading Lab initially simulated.
- Inputs: `investment-experience/report-source.md`, `investment-experience/claim-ledger.md`; minimum source paths are cited in the report.
- Changed files: documentation only at research handoff.
- Output: GPT-5.5 owns external inline mockup plus `experiment.md`/`experiment-checks.mjs`; Sol owns `data-strategy-experiment.md`/`data-strategy-checks.mjs` and private external data snapshots. Neither may edit production or the other's files.
- Verification: exact-source/code inspection; no live browser/strategy result claimed.
- Open risks: 2026 NSE operational ZIP not inspected, broker/data rights and live readiness unconfirmed. Synthetic results cannot establish profitability.
- Next action: independent QA after both experiment batons; user concept approval afterward.
- Do not revisit: no production/live authorization; one valuation for leased ownership; quotes never replace actual fills/charges.

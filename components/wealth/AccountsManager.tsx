...
export default function AccountsManager({ items }: { items: Account[] }) {
  return (
    <WealthManager<Account>
      items={items} fields={fields} addLabel="Add account" dialogTitle="account" emptyTitle="No accounts yet"
      emptyDescription="Add a bank, cash, pension or deposit account to track it in your net worth."
      toValues={(account) => ({ name: account.name, institution: account.institution, type: account.type, last4: account.last4 ?? "", currency: account.currency, balance: String(account.balance) })}
      onCreate={(values) => createAccount({ name: values.name, institution: values.institution, providerId: findFinancialProvider(values.institution)?.id ?? null, type: values.type as AccountType, currency: values.currency, balance: Number(values.balance) || 0, last4: values.last4 || null })}
      onUpdate={(id, values) => updateAccount(id, { name: values.name, institution: values.institution, providerId: findFinancialProvider(values.institution)?.id ?? null, type: values.type as AccountType, currency: values.currency, balance: Number(values.balance) || 0, last4: values.last4 || null })}
      onDelete={deleteAccount}
      renderRow={(account) => (
        <div className="flex items-center gap-3">
          <ProviderMark providerId={account.providerId} institution={account.institution} />
          <div className="min-w-0"><p className="truncate text-sm font-semibold text-gray-100">{account.name}</p><p className="truncate text-xs text-gray-500">{account.institution || ACCOUNT_TYPE_LABELS[account.type]}{account.last4 ? ` ••${account.last4}` : ""} · {ACCOUNT_TYPE_LABELS[account.type]}</p></div>

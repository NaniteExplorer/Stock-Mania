    ...
    <section className="panel overflow-hidden">
      <div className="flex flex-wrap items-start justify-between gap-4 border-b border-gray-600 p-5">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">Upload a holdings export (CSV, XLSX or PDF). Pick the asset type, then choose your file — Groww/INDmoney/Zerodha for stocks, CoinDCX for crypto, SafeGold for digital gold. Matched by symbol; re-imports just refresh quantities &amp; prices.</p>
        </div>
        <div className="flex items-center gap-1.5 text-xs text-green-500"><LockKeyhole className="h-3.5 w-3.5" /> Stored privately</div>
      </div>
      <div className="grid gap-4 p-5 sm:grid-cols-[180px_auto_1fr] sm:items-end">
        <div className="min-w-0">
          <label className="form-label text-xs">Asset type</label>
          <select
            value={kind}
            onChange={(event) => setKind(event.target.value as InvestmentKind)}
        ...
        <button
          ...
          disabled={pending}
          className="btn-brand h-11 w-full shrink-0 justify-center whitespace-nowrap px-5 sm:w-auto"
        >
